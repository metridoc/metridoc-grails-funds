...
class FundsController {
	...
	private getLedgerOptions(){
		def ledgers = config.AppConfig.ledgers
		def ledgerOptions = [];
	...
	private getReportPageModel(){
		...
		return [sortByOptions:config.AppConfig.sortByOptions,
			ledgerOptions: getLedgerOptions(),
			vendorList: config.AppConfig.vendors,
			fundList: fundList]
	}
	...
	private static getLedgerDates(ledger){
		def dateFormat = config.AppConfig.ledgerDateFormat;
	...
	def fund_report(ReportCommand cmd){
		if(!cmd.hasErrors()){	
			...
			if(cmd.reportType == ReportCommand.SUMMARY_REPORT_TYPE){
				def ledger = config.AppConfig.ledgers.get(cmd.ledgerSumm);
				...
				println config.AppConfig.ledgers.getClass()
